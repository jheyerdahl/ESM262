...
For this assignment, you'll import, tidy, and analyze the [California gazetteer dataset](https://geonames.usgs.gov/docs/stategaz/CA_Features_20180401.zip) that we used as an example in the [Data Wrangling](https://ucsb-bren.github.io/esm262/wrangle/) course sessions.
...
1. Write the `gaz` tibble to a CSV file (using `"|"` as a delimiter).
...
1. What are the fractions of the total number of features in each county that are natural? man-made?
    - Hint: Construct a tibble with two columns, one containing all possible feature classes (see "[Feature Class Definitions](https://geonames.usgs.gov/apex/f?p=gnispq:8)"), and another containing the string "natural" or "man-made", which you assign (it's pretty obvious.) Then join this tibble to the gazetteer tibble.
